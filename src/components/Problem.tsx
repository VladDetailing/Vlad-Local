      ...
      <div className="container mx-auto px-6 max-w-5xl">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-6 mb-8 md:mb-12">
          {problems.map((problem, index) => (
            <div
              key={index}
              className="flex items-center gap-3 md:gap-4 p-4 md:p-6 bg-gradient-to-r from-red-900/20 to-gray-900 border border-red-500/30 rounded-xl"
            >
              <AlertCircle className="w-6 h-6 md:w-8 md:h-8 text-red-400 flex-shrink-0" />
              <span className="text-base sm:text-lg md:text-xl text-gray-300">{problem}</span>
            </div>
          ))}
        </div>

        <div className="bg-gradient-to-br from-blue-950 to-gray-900 p-6 md:p-10 rounded-2xl border-2 border-blue-500 text-center">
